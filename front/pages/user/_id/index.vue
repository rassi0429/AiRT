  ...
  div
    ...
    div#content
      div.user-header.is-flex
        ...
        div.ml-auto()
          a.has-text-white(:href="`/user/${uid}/favs`") My Favorite
      div#imageGrid
...
export default {
  ...
  async asyncData({params, query}) {
    try {
      const {data} = await axios.get(`https://api.airt.cc/v1/user/${params.id}`)
      if (query.modal) {
  ...
  methods: {
    ...
    async getUserPhoto() {
      const {data} = await axios.get(`${this.endpoint}/v1/user/${this.$route.params.id}/photos?nsfw=${this.r18 || false}`)
      return data
